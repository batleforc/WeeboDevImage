...
echo "[INFO] Add font"
export WORKBENCH_PATH="/checode/checode-linux-libc/ubi9/out/vs/code/browser/workbench"
if ln -s /usr/share/fonts/ $WORKBENCH_PATH/fonts; then
  echo "[INFO] Font add successful"
else
  echo "[WARN] Couldn't add fonts"
fi

# Add FiraCode Nerd Font if not already present
CSS_FILE="$WORKBENCH_PATH/workbench.css"
FONT_NAME="FiraCode Nerd Font"
if [ -f "$CSS_FILE" ] && ! grep -qF "$FONT_NAME" "$CSS_FILE"; then
  echo "[INFO] Injecting '$FONT_NAME' into workbench.css"
  echo "" >> "$CSS_FILE"
  echo "@font-face { font-family: \"$FONT_NAME\"; src: url('fonts/FiraCodeNerdFont-Medium.ttf') format('truetype'); }" >> "$CSS_FILE"
else
  echo "[INFO] Font '$FONT_NAME' already present in workbench.css or file not found."
fi

if ! [ -f /checode/checode-linux-libc/ubi9/bin/remote-cli/code ]; then
  ln -s /checode/checode-linux-libc/ubi9/bin/remote-cli/code-oss /checode/checode-linux-libc/ubi9/bin/remote-cli/code
fi
