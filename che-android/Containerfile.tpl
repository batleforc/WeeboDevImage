...
RUN curl -fsSL "https://dl.google.com/android/repository/commandlinetools-linux-${ANDROID_CMDLINE_TOOLS_VERSION}_latest.zip" -o /tmp/cmdline-tools.zip && \
    ...
    # cmdline-tools 22 avdmanager opens <sysimg>/devices.xml without checking it
    # exists, and Google ships none in this image; an empty device list keeps
    # AVD creation from logging two bogus "Could not load devices" errors
    printf '<?xml version="1.0"?>\n<d:devices xmlns:d="http://schemas.android.com/sdk/devices/7"/>\n' \
    > ${ANDROID_SDK_ROOT}/system-images/android-${ANDROID_API}/google_apis/x86_64/devices.xml && \
...
# 4 cores so first-boot dex2oat doesn't starve system_server's main thread
# past the 60s Android watchdog (the devfile cpuLimit is sized to match)
ENV EMULATOR_CORES=4
ENV EMULATOR_PARTITION_MB=4096
# scales guest watchdog/ANR timeouts; seeded into /data/local.prop after the
# first boot (see entrypoint), where init picks it up on every later boot
ENV HW_TIMEOUT_MULTIPLIER=10
ENV LCD_WIDTH=720
ENV LCD_HEIGHT=1280
ENV LCD_DENSITY=320
