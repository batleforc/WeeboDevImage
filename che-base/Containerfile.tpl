...
RUN echo 'export PATH="${HOME}/.krew/bin:$PATH"' >> ${GLOBALS_BASHRC}
...
## Install Helm
RUN curl -fsSLo helm-${HELM_VERSION}-linux-amd64.tar.gz \
...
## Install Taskfile
RUN curl -fsSLo task_linux_amd64.tar.gz \
...
## Install k9s
RUN curl -fsSLo k9s_Linux_amd64.tar.gz \
...
## Install Cocogitto
RUN curl -fsSLo cocogitto-${COCOGITTO_VERSION}-x86_64-unknown-linux-musl.tar.gz \
  https://github.com/cocogitto/cocogitto/releases/download/${COCOGITTO_VERSION}/cocogitto-${COCOGITTO_VERSION}-x86_64-unknown-linux-musl.tar.gz && \
...
RUN curl -fsSLo gitleaks_${GITLEAKS_VERSION}_linux_x64.tar.gz \
...
RUN echo 'alias load-priv="gpg --import /etc/gpgkey/keypriv.asc"' >> ${GLOBALS_BASHRC}
RUN echo 'alias load-pub="gpg --import /etc/gpgkey/keypub.asc"' >> ${GLOBALS_BASHRC}

## Config Starship Shell Prompt
RUN curl -sS https://starship.rs/install.sh | sh -s -- -y
ENV STAR_NO="true"
RUN echo '[ $STAR_NO != "false" ] && eval "$(starship init bash)" ' >> ${GLOBALS_BASHRC}
...
## subuid/subgid: files are writable by group so entrypoint can update them dynamically
RUN touch /etc/subgid /etc/subuid && \
    chmod g=u /etc/subgid /etc/subuid /etc/passwd /etc/group

## newuidmap/newgidmap need cap_setuid/cap_setgid to work in user namespaces
...
RUN mkdir -p /home/user && ls /home && stow . -t /home/user/ -d /home/tooling/ --no-folding
